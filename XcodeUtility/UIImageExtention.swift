...
public extension UIImage {
    ...
    func imageWithSize(rect:CGRect) -> UIImage
    {
        
        let rect = AVMakeRect(aspectRatio: self.size,insideRect: rect)
        UIGraphicsBeginImageContextWithOptions(size, false, 0);
        self.draw(in: rect);
    ...
    /**
     ...
     - Returns A new image
     */
    class func image(fromURL url: String, placeholder: UIImage? = nil, shouldCacheImage: Bool = true, closure: @escaping (_ image: UIImage?,_ url:String) -> (), expire: @escaping (_ isExpire: Bool?) -> ()) -> UIImage? {
        // From Cache
        if shouldCacheImage {
            let imgName = url.components(separatedBy: "/").last ?? url
            if let image = UIImage.shared.object(forKey: imgName as AnyObject) as? UIImage {
                //                print("image found in Cache with key: " + imgName as AnyObject)
                closure(image,url)
                return image
            }
        }
        ...
        if let nsURL = URL(string: url) {
            ...
            DispatchQueue.global(qos: .background).async {
                ...
                let session = URLSession.shared//(configuration: URLSessionConfiguration.default)
                session.getTasksWithCompletionHandler({ (sesstionsTasks,sesstionsUploadTasks, sesstionsDownloadTasks) in
//                    print("=============================\n find sesstionsTasks ==== \(sesstionsTasks)")
//                    print("=============================\n find sesstionsUploadTasks ==== \(sesstionsUploadTasks)")
//                    print("=============================\n find sesstionsDownloadTasks ==== \(sesstionsDownloadTasks)")

                    if let task = sesstionsTasks.first(where: { (firstTask) -> Bool in
                        return firstTask.originalRequest?.url?.absoluteString == url
                    }){
//                        print("=============================\n find Request ==== \(url)")
                ...
                print("Error downloading image: \(nsURL) \(String(describing: error)) ")
                    ...
                    if let data = data, let image = UIImage(data: data) {
                        if shouldCacheImage {
                            let imgName = url.components(separatedBy: "/").last ?? url
                            UIImage.shared.setObject(image, forKey: imgName as AnyObject)
                        }
                        DispatchQueue.main.async {
                            closure(image,url)
                        }
    ...
    func normalizedImage() -> UIImage {
        ...
        rect.size = self.size  //self.size
        self.draw(in: rect)
        let retVal = UIGraphicsGetImageFromCurrentImageContext()
        UIGraphicsEndImageContext()
        
        return cropToBounds(image: retVal!, width: Double(self.size.width), height: Double(self.size.height))
    }
    func rotateImage(orientation: UIImage.Orientation? = nil)->UIImage
    {
        let image:UIImage = self
        var rotatedImage = UIImage()
        switch orientation ?? image.imageOrientation
        {
        ...
        case UIImage.Orientation.leftMirrored:
            rotatedImage = UIImage(cgImage:image.cgImage!, scale: 1, orientation:UIImage.Orientation.downMirrored)
        default:
